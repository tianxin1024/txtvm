...
def _load_lib():
    ...
    # DMatrix function
    lib.TXTVMGetLastError.restype = ctypes.c_char_p
    return lib
...
def check_call(ret):
    ...
    """
    if ret != 0:
        raise TXTVMError(py_str(_LIB.TXTVMGetLastError()))
...
def ctypes2docstring(num_args, arg_names, arg_types, arg_descs, remove_dup=True):
    """Convert ctypes returned doc string information info parameters docstring.

        num_args : nn_uint
            Number of arguments.

        arg_names : ctypes.POINTER(cytpes.c_char_p)
            Argument names.

        arg_types : ctypes.POINTER(ctypes.c_char_p)
            Argument type infomation.
