...
class NodeBase(object):
    ...
    def __del__(self):
        check_call(_LIB.TXTVMNodeFree(self.handle))
...
def _push_arg(arg):
    a = ArgVariant()
    if arg is None:
        _LIB.TXTVMPushStack(a, ctypes.c_int(kNull))
    elif isinstance(arg, NodeBase):
        a.v_handle = arg.handle
        _LIB.TXTVMPushStack(a, ctypes.c_int(kNodeHandle))
    elif isinstance(arg, int):
        a.v_long = ctypes.c_long(arg)
        _LIB.TXTVMPushStack(a, ctypes.c_int(kLong))
    elif isinstance(arg, Number):
        a.v_double = ctypes.c_double(arg)
        _LIB.TXTVMPushStack(a, ctypes.c_int(kDouble))
    elif isinstance(arg, string_types):
        a.v_str = c_str(arg)
        _LIB.TXTVMPushStack(a, ctypes.c_int(kStr))
    else:
...
def _init_function_module(root_namespace):
    ...
    check_call(_LIB.TXTVMListFunctionNames(ctypes.byref(size),
                                            ctypes.byref(plist)))
    op_names = []
